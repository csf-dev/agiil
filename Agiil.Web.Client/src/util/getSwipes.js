...
export default function getSwipes(element : HTMLElement, allowMouseSwipes : bool = false) : Observable<SwipeEvent> {
    ...
    return events.start.pipe(concatMap(getTimedSwipeEndingFactory(events)));
}

function getTimedSwipeEndingFactory(events : TouchEventsForElement) {
    return function getTimedSwipeEnding(startCoords : Coordinate) {
